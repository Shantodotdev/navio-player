...
export function PlayerBar() {
  ...
  return (
    <div className="w-full h-24 bg-[#050507]/98 backdrop-blur-2xl border-t border-white/5 px-8 flex items-center justify-between shadow-[0_-15px_30px_-15px_rgba(0,0,0,0.8)] z-50 shrink-0">
      ...
      <div className="flex items-center gap-4 w-1/3">
        <div className="w-14 h-14 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center overflow-hidden group relative shrink-0">
          {currentTrack?.cover_cache_path ? (
            <img
              src={`http://localhost:18080/stream/${currentTrack.cover_cache_path}`}
              alt=""
      ...
      <div className="flex flex-col items-center gap-2 w-1/3">
        <div className="flex items-center gap-5">
          ...
          <button
            ...
            className="w-10 h-10 rounded-full bg-brand hover:bg-brand-light flex items-center justify-center text-zinc-200 transition-all shadow-md shadow-brand-glow transform active:scale-95 animate-none"
          >
            {isPlaying ? (
      ...
      <div className="flex items-center justify-end gap-5 w-1/3">
        <div className="flex items-center gap-3">
          ...
          <div className="w-24 h-1 bg-white/10 rounded-full cursor-pointer relative group">
            <div className="h-full bg-white/40 group-hover:bg-brand rounded-full w-2/3"></div>
          </div>
        </div>
